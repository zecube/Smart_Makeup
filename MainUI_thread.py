    ...
    def run(self):
        ...
        while self.power :
            ...
            # self.change_start_signal.emit(detect_face, self.w, self.h)
                if w >= 400 and h>= 400:
                    detect_face = 1
                    if self.pre_detect_face == detect_face:
                        self.change_start_signal.emit(self.pre_detect_face)
                    else:
                        self.pre_detect_face = detect_face
                else:
                    self.pre_detect_face = 0
                    self.change_start_signal.emit(self.pre_detect_face)
            self.wait(5000)
...
class MakeUpThread(QThread):
    ...
    def __init__(self, skin_value = False, skin_ref_img = None, s_mix=0,
    ...
    def run(self):
        ...
        while self.power == True:
            ...
            if self.pause:
                ...
                def draw_ALL_FACE_OVAL_mask(mask, colors):      #얼굴
                    ...
                    if results.multi_face_landmarks:
                        ...
                        # mask =utils.fillPolyTrans(mask, [mesh_coords[p] for p in LIPS], (0,0,0), opacity=1 )   # (입술+입술 내부) 포함해서 뺌
                        mask =utils.fillPolyTrans(mask, [mesh_coords[p] for p in LIPS], (0,0,0), opacity=1 )
                        # 눈 뺄 시
                ...
                def draw_ALL_FACE_OVAL(mask, colors, ref):      #얼굴
                    # Inialize hair segmentation model / 머리카락 세분화 모델을 초기화합니다.
                    hair_segmentation = HairSegmentation()
                    ...
                    if results.multi_face_landmarks:
                        ...
                        # mask =utils.fillPolyTrans(mask, [mesh_coords[p] for p in LEFT_EYEBROW], (0,0,0), opacity=1 )
                        # mask =utils.fillPolyTrans(mask, [mesh_coords[p] for p in RIGHT_EYEBROW], (0,0,0), opacity=1 )
                        ref = makeup_ref_img.draw_FACE_example(ref, utils.WHITE)
                        matched = match_histograms(masked,ref, channel_axis= -1)
                        s_weighted_img = cv2.addWeighted(matched, self.s_mix/100, masked, 1-(self.s_mix/100), 0)
                        
                        return s_weighted_img, mask
                    
                def draw_FACE_OVAL(mask, colors, ref_img):      #얼굴
                    # Inialize hair segmentation model / 머리카락 세분화 모델을 초기화합니다.
                    hair_segmentation = HairSegmentation()
                ...
                #화장 내의 버튼이 눌린 상태
                    ...
                    if self.skin_value and self.mouth_value:
                        if results.multi_face_landmarks:
                            fore, face_mask_copy = draw_ALL_FACE_OVAL(face_mask, utils.WHITE, self.skin_ref_img)
                            back, face_mask_copy = draw_ALL_FACE_OVAL(face_mask, utils.WHITE, self.mouth_ref_img)
                            face_mask = draw_ALL_FACE_OVAL_mask(face_mask, utils.WHITE)
                            foreground = fore.astype(float)
                            ...
                            #화장한 얼굴
                            foreground = outImage.astype(float)
                            #얼굴을 제외한 배경
                            background = frame.astype(float)
                            alpha_mask = face_mask_copy.astype(float)/255
                            foreground = cv2.multiply(alpha_mask, foreground)
                            background = cv2.multiply(1.0 - alpha_mask, background)
                            outImage = cv2.add(foreground, background)

                            #opencv에서 기본적으로 사용하는 데이터 형식으로 변경
                            out = outImage.astype(np.uint8)
                            output = out
...
class Ui_MainWindow(object):
    ...
    def setupUiOn(self, MainWindow):
        ...
        displayoff = QIcon("data\\button_icon\\turnoff2.png")
        ...
        self.aircleanerWidget.setGeometry(QRect(1700, 300, self.aircleanerbaseWidth,170))
    ...
    def AirCleanerFunc(self, MainWindow):
        if self.btnAircleanerShow.isChecked():
            ...
            self.aircleanerAni.setStartValue(QRect(1700, 300, self.aircleanerbaseWidth,170))
            self.aircleanerAni.setEndValue(QRect(1700, 300, self.airclenerextendedWidth,170))
            ...
            self.btnAircleanerShow.setStyleSheet("QPushButton { background-color: #99CCFF; border-radius: 30px}")
            self.aircleanerAni.setDuration(800)
            self.aircleanerAni.setStartValue(QRect(1700, 300, self.airclenerextendedWidth,170))
            self.aircleanerAni.setEndValue(QRect(1700, 300, self.aircleanerbaseWidth,170))
            self.aircleanerAni.start()
   ...
   #화장결과 숨기기 / 화장결과 보이기
    ...
    def s_color1(self):
        self.face_make_thread.pause = False
        img = cv2.imread('makeup_img\skin\spring_skin.png') #임시
    ...
    def m_color1(self):
        self.face_make_thread.pause = False
        img = cv2.imread('makeup_img\lip\spring_lip_orange.jpg', cv2.IMREAD_UNCHANGED)
    ...
    def m_color2(self):
        self.face_make_thread.pause = False
        img = cv2.imread('makeup_img\lip\summer_lip.jpg', cv2.IMREAD_UNCHANGED)
        self.face_make_thread.mouth_ref_img =img
        self.face_make_thread.mouth_value = True
        self.face_make_thread.pause = True
    ...
    def m_color3(self):
        self.face_make_thread.pause = False
        img = cv2.imread('makeup_img\lip\\fall_lip.png', cv2.IMREAD_UNCHANGED)
        self.face_make_thread.mouth_ref_img =img
        self.face_make_thread.mouth_value = True
        self.face_make_thread.pause = True
    ...
    def m_color4(self):
        self.face_make_thread.pause = False
        img = cv2.imread('makeup_img\lip\winter_lip.jpg', cv2.IMREAD_UNCHANGED)
        self.face_make_thread.mouth_ref_img =img
        self.face_make_thread.mouth_value = True
        self.face_make_thread.pause = True
    ...
    def m_color5(self):
        self.face_make_thread.pause = False
        img = cv2.imread('makeup_img\lip\winter_lip2.jpg', cv2.IMREAD_UNCHANGED)
        self.face_make_thread.mouth_ref_img =img
        self.face_make_thread.mouth_value = True
        self.face_make_thread.pause = True
